...
def profile_me(request):
    user = request.user
    # Support both clients and staff. Return minimal role info for staff so
    # frontend can distinguish and show/hide manager-only UI.
    ...
    # If user is staff, return role only (frontend can render manager UI)
    if hasattr(user, "staff"):
        return Response({"username": user.username, "role": "staff"})

    return Response({"error": "User type unknown"}, status=400)
...
# =========================================================
# 6. STAFF: ANALYZE USER CHURN RISK (NEW)
# =========================================================
...
def analyze_user_churn(request, user_id):
    """
    Staff endpoint to analyze any user's churn risk.
    POST /api/users/{user_id}/analyze/
    """
    # 1. 验证权限: 只有staff可以调用
    if not hasattr(request.user, "staff"):
        return Response({"error": "Permission denied. Staff only."}, status=403)
